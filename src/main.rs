...
use mite::capture::{WindowCapturePreference, WindowSelector, list_capturable_windows};
use mite::config::AppConfig;
use mite::doctor::DoctorReport;
use mite::{artifact, eval, interactive, storage_cleanup};
...
struct WatchArgs {
    ...
    enable_eval_hotkey: Option<mite::hotkey::HotkeyCombo>,
...
fn cmd_eval(
    ...
    if !report.passed && !allow_failures {
        bail!(
...
fn cmd_watch(config_path: &Path, int8: Int8Override, args: WatchArgs) -> Result<()> {
    ...
    let eval_hotkey = match args.enable_eval_hotkey {
        Some(combo) => Some(interactive::EvalHotkeyRequest {
            ...
            output_dir: args
                .eval_capture_dir
                .unwrap_or_else(mite::eval_capture::default_capture_root),
        }),
        None => {
